                ...
                <div class="card h-100 fixed-image-wrapper">
                    ...
                    <div class="d-flex justify-content-between align-items-center px-3 mb-2">
                      ...
                      <div class="d-flex gap-2">
                        <button type="button" class="fas fa-info-circle" style="color: black; border: none; background: none;"
                    ...
                    data-fungsi="{{ $item->deskripsi }}">
                        ...
                        <button type="button" class="btn btn-warning barang-to-keranjang" id="" data-barangid="{{ $item->id }}">Rent</button>
                      </div>
                    </div>
...
{{-- modal2 --}}
<div class="modal fade" id="formKeranjang" tabindex="-1" aria-labelledby="formPopupLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="torentdetail">
          ...
          <div class="mb-3">
            <label for="rents_id" class="form-label">List Keranjang</label>
            <select class="form-control" id="rents_id" name="rents_id" required>
        ...
        $(document).ready(function () {
          ...
          if (!queryParam) {
            Swal.fire({
              ...
              confirmButtonText: 'Buat Keranjang',
              cancelButtonText: 'Sudah Punya',
              reverseButtons: true, // Membalik posisi tombol (OK di kanan)
            }).then((result) => {
              if (result.isConfirmed) {
                $('#openFormModal').on('click', function () {
                  popupKeranjang.modal('hide');
                  formKeranjang.modal('show');
                });
                // Aksi jika tombol "Buat Keranjang" diklik
                document.getElementById('openFormModal').click();
              } else if (result.dismiss === Swal.DismissReason.cancel) {
                // Aksi jika tombol "Nanti" diklik
                console.log('Pengguna memilih Nanti.');
              }
            });
          }

          // Kode untuk proses lain (misalnya form submission)
          $('#buatKeranjangForm').on('submit', function (e) {
            ...
            $.ajax({
              url: '{{ route('rent.store') }}', // URL route Laravel
              type: 'POST',
              data: formData,
              success: function(response) {
                formKeranjang.modal('hide');
                Swal.fire({
                  icon: response.icon || 'success',
                  title: response.title || 'Sukses!',
                  text: response.text || 'Keranjang berhasil disimpan.',
                  timer: 3000,
                  showConfirmButton: false
                });
                $('#buatKeranjangForm')[0].reset();
              },
              error: function(xhr) {
                ...
                Swal.fire({
                  icon: 'error',
                  title: 'Error!',
                  text: 'Terjadi kesalahan, silakan periksa kembali data Anda.',
                  timer: 3000,
                  showConfirmButton: false
                });
              }
          ...
          // Kode lainnya yang sudah ada tetap berjalan seperti biasa
          $('.barang-to-keranjang').on('click', function () {
            var barangs_id = $(this).data('barangid');
            ...
            $.ajax({
              ...
              success: function (response) {
                ...
                $('#jumlahbarang').attr("max", response.data.stok_barang);
              }
            });

            $.ajax({
              type: "GET",
              url: `{{ url('/rent/keranjang-ajax') }}`,
              cache: false,
              success: function (response) {
                const data = response.data;
                const select = $('#rents_id');
                select.empty();
                select.append('<option value="">Pilih Keranjang</option>');
                const options = data.map(item => {
                  return `<option value="${item.id}">${item.nama_keranjang}</option>`;
                }).join('');
          ...
          $('#torentdetail').on('submit', function (e) {
            e.preventDefault(); // Mencegah reload halaman
            ...
            // Kirim data ke server menggunakan AJAX
            $.ajax({
              url: '{{ route('rent.storeBarang') }}', // URL route Laravel
              type: 'POST',
              data: formData,
              success: function(response) {
